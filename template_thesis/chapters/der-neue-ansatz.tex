...
Pinia verfügt über ein Plugin System. Über dieses bekommt man unter anderem Zugriff auf die Zustände und Actions der aktiven Stores. Die Plugins werden beim Start der Applikation über die \textit{Pinia.use} API registriert.

Die Übergangsmap hat die gleiche Struktur wie die bei Redux. Allerdings wird das Feld \textit{actionTypes} zu \textit{action} umbenannt, da die Actions hierbei keine POJOs sind und für den Namen der Action in dem \textit{actions} Objekt stehen.
